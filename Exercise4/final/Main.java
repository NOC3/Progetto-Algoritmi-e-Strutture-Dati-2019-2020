...
public class Main {
  public static void main(String[] args) {
    ...
    // catch of stackoverflow, needed to easily see execution-times in terminal
    try {
      g.decompose(1, -1, 1);
    } catch (StackOverflowError s) {
      System.out.println("Reached stack maximum size...");
      System.exit(0);
    }

    // read and solve query
    try {
      BufferedWriter bw = new BufferedWriter(new FileWriter(fileName.replace("input.txt", "output.txt"))); // substitution
                                                                                                           // in args[0]
      ...
      while (i < queryN) {
        ...
        if (g.query(Integer.parseInt(parameters[0]), Integer.parseInt(parameters[1])) > Integer
